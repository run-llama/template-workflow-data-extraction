...
agent_name = os.getenv("LLAMA_DEPLOY_DEPLOYMENT_NAME") or "extraction-agent"
...
def get_extract_agent() -> ExtractionAgent:
    ...
    try:
        existing = extract_api.get_agent(agent_name)
        ...
        if e.status_code == 404:
            return extract_api.create_agent(
                name=agent_name, data_schema=MySchema, config=config
            )
        else:
            raise
